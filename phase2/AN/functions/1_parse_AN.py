...
def parse_to_json(raw_data_filepath_excel):
    print("Parsing data file: {}". format(raw_data_filepath_excel))
    # load attribute df from csv file

    AN_attribute_df = pd.read_csv(ATTRIBUTE_GROUND_TRUTH)
    ...
    sheet_to_df_map = pd.read_excel(raw_data_filepath_excel, sheet_name=None)
    ...
    for sheet_id in sheet_ids:
        ...
        num_concepts = int ( ( len(col) - PRESET[DATASET_ID] ) / 3 )
        concept_count += num_concepts
        # print(num_concepts)

        offset = OFFSET[DATASET_ID]
...
def save_data(data_dict, filepath):
    ...
    print("Saved parsed data at: {}".format(filepath))

if __name__ == "__main__":
    data_dict = parse_to_json(RAW_DATA_FILEPATH_EXCEL)
    save_data(data_dict, RAW_DATA_FILEPATH_JSON)
